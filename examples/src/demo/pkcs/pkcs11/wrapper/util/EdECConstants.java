...
public class EdECConstants {
  ...
  public static boolean isEdwardsOrMontgomeryCurve(
  ...
  public static ASN1ObjectIdentifier getCurveOid(String curveName) {
    if (X25519.equalsIgnoreCase(curveName)
        || id_X25519.getId().equals(curveName)) {
      return id_X25519;
    } else if (X448.equalsIgnoreCase(curveName)
        || id_X448.getId().equals(curveName)) {
      return id_X448;
    } else if (Ed25519.equalsIgnoreCase(curveName)
        || id_Ed25519.getId().equals(curveName)) {
      return id_Ed25519;
    } else if (Ed448.equalsIgnoreCase(curveName)
        || id_Ed448.getId().equals(curveName)) {
      return id_Ed448;
    } else {
      return null;
    }
