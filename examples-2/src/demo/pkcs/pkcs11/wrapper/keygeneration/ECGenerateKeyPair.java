...
public class ECGenerateKeyPair extends TestBase {
  ...
  private void main0(Token token, Session session)
    ...
    KeyPair generatedKeyPair = session.generateKeyPair(
        ...
        privateKeyTemplate);
    ECDSAPublicKey generatedPublicKey =
        (ECDSAPublicKey) generatedKeyPair.getPublicKey();
    ECDSAPrivateKey generatedPrivateKey =
        (ECDSAPrivateKey) generatedKeyPair.getPrivateKey();
    ...
    try {
      ...
      LOG.info("##################################################");
      ECDSAPublicKey exportablePublicKey = generatedPublicKey;
      byte[] encodedPoint =
      ...
      ECDSAPublicKey exportPublicKeyTemplate = new ECDSAPublicKey();
      exportPublicKeyTemplate.getId().setByteArrayValue(id);

      session.findObjectsInit(exportPublicKeyTemplate);
      iaik.pkcs.pkcs11.objects.Object[] foundPublicKeys =
