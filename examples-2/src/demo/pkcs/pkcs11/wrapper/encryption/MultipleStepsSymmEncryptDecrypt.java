...
public abstract class MultipleStepsSymmEncryptDecrypt extends TestBase {
  ...
  protected abstract SecretKey getKeyTemplate();
  ...
  private void main0(Token token, Session session) throws TokenException, IOException {
    ...
    SecretKey keyTemplate = getKeyTemplate();
    ...
    for (int i = 0; i < rawData.length; i += 64) {
      ...
      byte[] part = Arrays.copyOfRange(rawData, i, i + inLen);
      byte[] res = session.encryptUpdate(part);
      if (res != null) {
        bout.write(res);
      }
    }
    ...
    for (int i = 0; i < encryptedData.length; i += 64) {
      ...
      byte[] part = Arrays.copyOfRange(encryptedData, i, i + inLen);
      res = session.decryptUpdate(part);
      if (res != null) {
        bout.write(res);
      }
    }
